...
try:
    ...
    print("Installing pyroomacoustics...")
    import subprocess
    import sys
...
def generate_chirp(duration, fs, f_start, f_end, t_start=0):
    ...
    chirp[start_idx:end_idx] = signal.chirp(t_chirp, f_start, t_chirp[-1], f_end, method='linear')
    return chirp
...
def create_test_signals(duration=4.0, fs=16000):
    ...
    signal2 = generate_chirp(duration, fs, f_start=2000, f_end=4000, t_start=2.0)
    
    # Signal 3: Pure tone in middle frequency range
    signal3 = generate_tone_burst(duration, fs, frequency=1200, t_start=1.0, burst_duration=1.5)
...
def plot_signals_and_spectrogram(signals, mic_signals, fs):
    ...
    if len(axes) > 2:
        ...
        im = ax_spec.pcolormesh(t_spec, f, 10*np.log10(Sxx + 1e-12), shading='gouraud')
